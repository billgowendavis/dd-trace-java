...
public class Agent {
  ...
  private static void maybeStartRemoteConfig(Class<?> scoClass, Object sco) {
    try {
      Field pollerField = scoClass.getField("configurationPoller");
      Object poller = pollerField.get(sco);
      if (poller == null) {
        log.debug("Remote config is not enabled");
        return;
      }
      Class<?> pollerCls = SHARED_CLASSLOADER.loadClass("datadog.remote_config.ConfigurationPoller");
