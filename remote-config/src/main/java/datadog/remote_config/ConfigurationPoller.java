...
public class ConfigurationPoller implements AgentTaskScheduler.Target<ConfigurationPoller> {
  ...
  public ConfigurationPoller(Config config,
  ...
  ConfigurationPoller(
    ...
    this.maxPayloadSize = config.getRemoteConfigMaxPayloadSize();
    Moshi moshi = new Moshi.Builder().add(Instant.class, new InstantJsonAdapter()).build();
  ...
  private void handleAgentResponse(ResponseBody body) {
    ...
    for (String configKey: configsToApply) {
      try {
        boolean res = processConfigKey(fleetResponse, configKey, inspectedConfigurationKeys, pollingRateHinter);
        if (res) {
          successes++;
        } else {
          failures++;
    ...
    if (successes == 0 && failures > 0) {
      throw new RuntimeException("None of the configuration data was successfully read and processed");
    }
  ...
  private void unapplyConfigs(List<String> configsToApply, PollingRateHinter hinter) {
    Set<String> activeProducts =
        configsToApply.stream().map(ConfigurationPoller::extractProductFromKey).collect(Collectors.toSet());
  ...
  private boolean processConfigKey(RemoteConfigResponse fleetResponse,
                                   String configKey,
    ...
    if (dl == null) {
      throw new ReportableException("Told to handle config key " + configKey +
          ", but the product " + productName + " is not being handled");
    }
    ...
    if (!maybeFileContent.isPresent()) {
      if (this.cachedTargetFiles.containsKey(configKey)) {
        throw new ReportableException("Told to apply config " + configKey +
   ...
   * @param inspectedConfigKeys the configurations that were applied to the tracer either
   *                            a) in this iteration or b) after a previous request, and the
   *                            configuration has not changed. The spec is unclear about what
   *                            happens to config_states if there is an error early enough that
   *                            no configurations are even available. The option taken is to
   *                            empty config_states (but not cached_target_files).
   ...
   */
  private void updateNextState(RemoteConfigResponse fleetResponse,
                               List<String> inspectedConfigKeys,
                               String errorMessage) {
    ...
    for (int i = 0; i < numInspectedConfigs; i++) {
      String configKey = inspectedConfigKeys.get(0);
    ...
    long version = targetsSigned.version;
    this.nextClientState.setState(
        version, configStates, errorMessage, null /* TODO: backend_client_state */);
  ...
  private static final Pattern EXTRACT_PRODUCT_REGEX = Pattern.compile("[^/]+(?:/\\d+)?/([^/]+)/[^/]+/config");
  ...
  private void loadFromFile(File file, DeserializerAndListener<?> deserializerAndListener) {
    ...
    try (InputStream inputStream =
      ...
      boolean res = deserializerAndListener.deserializeAndAccept(buffer, PollingRateHinter.NOOP);
      if (!res) {
  ...
  private static class DeserializerAndListener<T> {
    ...
    DeserializerAndListener(ConfigurationDeserializer<T> deserializer,
                            ConfigurationChangesListener<T> listener) {
      this.deserializer = deserializer;
      this.listener = listener;
    }

    boolean deserializeAndAccept(byte[] bytes, PollingRateHinter hinter) {
